...
const AnimatedIcon: React.FC<AnimatedIconProps> = ({ name, size = 24, color = "white", onPress }) => {
...
export default function FamilyCall() {
  ...
  const saveContact = () => {
    ...
    if (editingContact) {
      setContacts((prev) =>
        prev.map((c) =>
          c.id === editingContact.id ? { ...c, name: nameInput.trim(), phone: phoneInput.trim() } : c
        )
      );
      Alert.alert("Updated", "Contact updated successfully!");
    } else {
      const newId = contacts.length ? Math.max(...contacts.map((c) => c.id)) + 1 : 1;
      setContacts((prev) => [{ id: newId, name: nameInput.trim(), phone: phoneInput.trim() }, ...prev]);
  ...
  const renderRightActions = (contact: Contact) => (
    <View style={styles.rightActionContainer}>
      ...
      <Pressable onPress={() => deleteContact(contact)} style={[styles.smallActionBtn, { marginTop: 10 }]}>
      ...
      <LinearGradient colors={["#F6FBF7", "#CFE4CF"]} style={styles.container}>
        <View style={styles.header}>
          <View style={{ flexDirection: "row", alignItems: "center",  }}>
            <Ionicons name="chevron-back" size={22} color="#0a3d2e" onPress={() => router.back()} />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ marginTop: 40, alignItems: "center" }}>
              <Text style={{ color: "#0a3d2e", opacity: 0.7 }}>No contacts — tap + to add</Text>
            </View>
          }
        />
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "flex-end", marginTop: 18 }}>
                <Pressable onPress={() => setModalVisible(false)} style={[styles.modalBtn, { backgroundColor: "#e6e6e6" }]}>
                  <Text>Cancel</Text>
                </Pressable>
                <Pressable onPress={saveContact} style={[styles.modalBtn, { marginLeft: 10, backgroundColor: "#0a3d2e" }]}>
